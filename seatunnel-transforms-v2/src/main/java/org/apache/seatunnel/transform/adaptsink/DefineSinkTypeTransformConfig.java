...
public class DefineSinkTypeTransformConfig implements Serializable {
    ...
    public static Optional<DefineSinkTypeTransformConfig> of(
            ReadonlyConfig config, TablePath tablePath) {
        if (null != config.get(MULTI_TABLES)) {
            return config.get(MULTI_TABLES).stream()
                    .filter(
                            tableTransforms ->
                                    tableTransforms.getTablePath().equals(tablePath.getFullName()))
                    .map(
                            tableTransform -> {
                                return new DefineSinkTypeTransformConfig(
                                        tableTransform.getColumns());
                            })
                    .findFirst();
        }
