...
public class OpengaussStreamingChangeEventSource
        implements StreamingChangeEventSource<OpengaussOffsetContext> {
    ...
    private static final Logger LOGGER =
            LoggerFactory.getLogger(OpengaussStreamingChangeEventSource.class);
    ...
    public OpengaussStreamingChangeEventSource(
            OpengaussConnectorConfig connectorConfig,
            Snapshotter snapshotter,
            OpengaussConnection connection,
            OpengaussEventDispatcher<TableId> dispatcher,
    ...
    @Override
    public void execute(ChangeEventSourceContext context, OpengaussOffsetContext offsetContext)
        ...
        if (!hasStartLsnStoredInContext) {
            offsetContext =
                    OpengaussOffsetContext.initialContext(connectorConfig, connection, clock);
        }
    ...
    private void commitMessage(OpengaussOffsetContext offsetContext, final Lsn lsn)
            throws SQLException, InterruptedException {
        lastCompletelyProcessedLsn = lsn;
    ...
    public void commitOffset(Map<String, ?> offset) {
        try {
            ...
            final Lsn commitLsn =
                    Lsn.valueOf((Long) offset.get(OpengaussOffsetContext.LAST_COMMIT_LSN_KEY));
            final Lsn changeLsn =
                    Lsn.valueOf(
                            (Long)
                                    offset.get(
