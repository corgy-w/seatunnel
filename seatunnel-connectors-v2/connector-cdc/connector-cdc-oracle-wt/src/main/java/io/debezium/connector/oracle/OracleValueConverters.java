...
public class OracleValueConverters extends JdbcValueConverters {
    ...
    public OracleValueConverters(OracleConnectorConfig config, OracleConnection connection) {
        ...
        this.unavailableValuePlaceholderString =
                new String(config.getUnavailableValuePlaceholder());
    }
            ...
            if (data instanceof TIMESTAMP) {
                ...
                data =
                        ZonedDateTime.ofInstant(
                                ts.timestampValue(connection.connection()).toInstant(),
        ...
        return convertValue(
                ...
                (r) -> {
                    if (data instanceof Number) {
                        ...
                        if (intervalHandlingMode
                                == OracleConnectorConfig.IntervalHandlingMode.STRING) {
                            r.deliver(
                        ...
                        INTERVALYM interval =
                                new INTERVALYM(value.substring(15, value.length() - 2));
    ...
    private void convertOracleIntervalYearMonth(Object data, ResultReceiver r) {
        ...
        for (int i = 1; i < interval.length(); i++) {
            if (interval.charAt(i) == '-') {
                ...
                final int month =
                        sign * Integer.parseInt(interval.substring(i + 1, interval.length()));
        ...
        return convertValue(
                ...
                (r) -> {
                    if (data instanceof Number) {
                        ...
                        INTERVALDS interval =
                                new INTERVALDS(value.substring(15, value.length() - 2));
    ...
    private void convertOracleIntervalDaySecond(Object data, ResultReceiver r) {
        ...
        if (m.matches()) {
            ...
            if (intervalHandlingMode == OracleConnectorConfig.IntervalHandlingMode.STRING) {
                double seconds =
                        (double) (sign * Integer.parseInt(m.group(5)))
                                + (double) Integer.parseInt(Strings.pad(m.group(6), 6, '0'))
                                        / 1_000_000D;
                r.deliver(
                        Interval.toIsoString(
                                0,
