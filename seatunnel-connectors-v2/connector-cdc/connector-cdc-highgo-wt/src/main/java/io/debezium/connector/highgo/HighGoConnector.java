...
/**
 * A Kafka Connect source connector that creates tasks which use HighGo streaming replication off a
 * logical replication slot to receive incoming changes for a database and publish them to Kafka.
 *
 * <h2>Configuration</h2>
 *
 * <p>This connector is configured with the set of properties described in {@link
 * HighGoConnectorConfig}.
 *
 * @author Horia Chiorean
 */
public class HighGoConnector extends RelationalBaseSourceConnector {

    private static final Logger LOGGER = LoggerFactory.getLogger(HighGoConnector.class);
    private Map<String, String> props;

    public HighGoConnector() {}
    ...
    public ConfigDef config() {
        return HighGoConnectorConfig.configDef();
    }
    ...
    protected void validateConnection(Map<String, ConfigValue> configValues, Configuration config) {
        ...
        final ConfigValue slotNameValue = configValues.get(HighGoConnectorConfig.SLOT_NAME.name());
        final ConfigValue pluginNameValue =
                configValues.get(HighGoConnectorConfig.PLUGIN_NAME.name());
        ...
        final HighGoConnectorConfig postgresConfig = new HighGoConnectorConfig(config);
        final ConfigValue hostnameValue =
                configValues.get(RelationalDatabaseConnectorConfig.HOSTNAME.name());
        // Try to connect to the database ...
        try (HighGoConnection connection = new HighGoConnection(postgresConfig.getJdbcConfig())) {
            try {
    ...
    protected Map<String, ConfigValue> validateAllFields(Configuration config) {
        return config.validate(HighGoConnectorConfig.ALL_FIELDS);
    }
}
