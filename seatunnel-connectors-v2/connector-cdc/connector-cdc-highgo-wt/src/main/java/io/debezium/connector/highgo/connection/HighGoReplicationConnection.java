...
/**
 * Implementation of a {@link ReplicationConnection} for HighGo. Note that replication connections
 * in PG cannot execute regular statements but only a limited number of replication-related
 * commands.
 *
 * @author Horia Chiorean (hchiorea@redhat.com)
 */
public class HighGoReplicationConnection extends JdbcConnection implements ReplicationConnection {
    ...
    private static Logger LOGGER = LoggerFactory.getLogger(HighGoReplicationConnection.class);

    private final String slotName;
    private final String publicationName;
    private final RelationalTableFilters tableFilter;
    private final HighGoConnectorConfig.AutoCreateMode publicationAutocreateMode;
    private final HighGoConnectorConfig.LogicalDecoder plugin;
    private final boolean dropSlotOnClose;
    private final HighGoConnectorConfig originalConfig;
     ...
     * @param typeRegistry registry with HighGo types
     ...
     */
    private HighGoReplicationConnection(
            HighGoConnectorConfig config,
            String slotName,
            String publicationName,
            RelationalTableFilters tableFilter,
            HighGoConnectorConfig.AutoCreateMode publicationAutocreateMode,
            HighGoConnectorConfig.LogicalDecoder plugin,
    ...
    private ServerInfo.ReplicationSlot getSlotInfo() throws SQLException, InterruptedException {
        try (HighGoConnection connection = new HighGoConnection(originalConfig.getJdbcConfig())) {
    ...
    protected void initPublication() {
        ...
        if (HighGoConnectorConfig.LogicalDecoder.PGOUTPUT.equals(plugin)) {
    ...
    // Temporary replication slots is a new feature of HighGo 10
    ...
    public ReplicationStream startStreaming(Lsn offset, WalPositionLocator walPosition)
        ...
        int maxRetries = config().getInteger(HighGoConnectorConfig.MAX_RETRIES);
        int delay = config().getInteger(HighGoConnectorConfig.RETRY_DELAY_MS);
        int tryCount = 0;
        while (true) {
            try {
        ...
        try {
            final ResultSet resultSet =
                    pgConnection()
                            ...
                            .executeQuery(
                                    String.format(
                                            "select * from pg_replication_slots where slot_name = '%s' and plugin = 'pgoutput';",
                                            slotName));
    ...
    private ReplicationStream createReplicationStream(
        ...
        try {
            ...
            if (e.getMessage().matches("(?s)ERROR: option .* is unknown.*")) {
                ...
                throw new ConnectException(
                        "The offset to start reading from has been removed from the database write-ahead log. Create a new snapshot and consider setting of HighGo parameter wal_keep_segments = 0.");
            } else {
                throw e;
            }
        }
    ...
    public synchronized void close(boolean dropSlot) {
        ...
        if (dropSlotOnClose && dropSlot) {
            ...
            // connection
            try (HighGoConnection connection =
                    new HighGoConnection(originalConfig.getJdbcConfig())) {
    ...
    protected static void defaultSettings(Configuration.Builder builder) {
        ...
        HighGoConnection.defaultSettings(builder);
    ...
    protected static class ReplicationConnectionBuilder implements Builder {
        ...
        private HighGoConnectorConfig.AutoCreateMode publicationAutocreateMode =
                HighGoConnectorConfig.AutoCreateMode.FILTERED;
        private HighGoConnectorConfig.LogicalDecoder plugin =
                HighGoConnectorConfig.LogicalDecoder.DECODERBUFS;
        private boolean dropSlotOnClose = DEFAULT_DROP_SLOT_ON_CLOSE;
        private Duration statusUpdateIntervalVal;
        private boolean doSnapshot;
        private TypeRegistry typeRegistry;
        private HighGoSchema schema;
        private Properties slotStreamParams = new Properties();

        protected ReplicationConnectionBuilder(HighGoConnectorConfig config) {
            assert config != null;
            this.config = config;
        }
        ...
        public ReplicationConnectionBuilder withPlugin(
                final HighGoConnectorConfig.LogicalDecoder plugin) {
            assert plugin != null;
            this.plugin = plugin;
            return this;
        }
        ...
        public ReplicationConnection build() {
            ...
            return new HighGoReplicationConnection(
                    config,
                    slotName,
                    publicationName,
                    tableFilter,
        ...
        @Override
        public Builder withSchema(HighGoSchema schema) {
            this.schema = schema;
            return this;
        }
    }
