...
import io.debezium.connector.highgo.HighGoType;
import io.debezium.connector.highgo.HighGoValueConverter;
import io.debezium.connector.highgo.OgOid;
 ...
 */
public class HighGoConnection extends JdbcConnection {
    ...
    private static Logger LOGGER = LoggerFactory.getLogger(HighGoConnection.class);
    ...
    protected static final ConnectionFactory FACTORY =
            JdbcConnection.patternBasedFactory(
                    ...
                    HighGoConnection.class.getClassLoader(),
                    JdbcConfiguration.PORT.withDefault(
                            HighGoConnectorConfig.PORT.defaultValueAsString()));
    ...
    private final HighGoDefaultValueConverter defaultValueConverter;
     ...
     * HighGoValueConverter}, and will provide its own {@link TypeRegistry}. Usually only one such
     * connection per connector is needed.
     *
     * @param config {@link Configuration} instance, may not be null.
     * @param valueConverterBuilder supplies a configured {@link HighGoValueConverter} for a given
     *     {@link TypeRegistry}
     */
    public HighGoConnection(
        ...
        if (Objects.isNull(valueConverterBuilder)) {
            ...
            final HighGoValueConverter valueConverter =
                    valueConverterBuilder.build(this.typeRegistry);
            this.defaultValueConverter =
                    new HighGoDefaultValueConverter(valueConverter, this.getTimestampUtils());
        }
    }
    ...
    public HighGoConnection(HighGoConnectorConfig config, TypeRegistry typeRegistry) {
        ...
        if (Objects.isNull(typeRegistry)) {
            ...
            final HighGoValueConverter valueConverter =
                    HighGoValueConverter.of(config, this.getDatabaseCharset(), typeRegistry);
            this.defaultValueConverter =
                    new HighGoDefaultValueConverter(valueConverter, this.getTimestampUtils());
        }
    }
     ...
     * HighGoConnection#fetchReplicationSlotInfo} call
     *
    ...
    private Optional<ColumnEditor> doReadTableColumn(
        ...
        if (columnFilter == null
            ...
            final HighGoType nativeType = getTypeRegistry().get(column.typeName());
    ...
    public HighGoDefaultValueConverter getDefaultValueConverter() {
    ...
    public <T extends DatabaseSchema<TableId>> Object getColumnValue(
        ...
        try {
            ...
            final HighGoType type = ((HighGoSchema) schema).getTypeRegistry().get(columnTypeName);
            ...
            switch (type.getOid()) {
                ...
                case OgOid.NUMERIC:
                    ...
                    Optional<SpecialValueDecimal> value = HighGoValueConverter.toSpecialValue(s);
     ...
     *
     * @param connectorConfig HighGo source config
     * @param connection HighGo Connection
     * @return Statement
     * @throws SQLException createStatement exception
     */
    public Statement readTableStatementHighGo(
            HighGoConnectorConfig connectorConfig, Connection connection) throws SQLException {
    ...
    @FunctionalInterface
    public interface HighGoValueConverterBuilder {
        HighGoValueConverter build(TypeRegistry registry);
    }
}
