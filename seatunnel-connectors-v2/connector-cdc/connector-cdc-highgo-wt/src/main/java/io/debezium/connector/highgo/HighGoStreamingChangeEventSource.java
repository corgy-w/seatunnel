...
public class HighGoStreamingChangeEventSource
    ...
    private static final Logger LOGGER =
            LoggerFactory.getLogger(HighGoStreamingChangeEventSource.class);
    ...
    private final HighGoConnection connection;
    private final HighGoEventDispatcher<TableId> dispatcher;
    private final ErrorHandler errorHandler;
    private final Clock clock;
    private final HighGoSchema schema;
    private final HighGoConnectorConfig connectorConfig;
    private final HighGoTaskContext taskContext;
    ...
    public HighGoStreamingChangeEventSource(
            HighGoConnectorConfig connectorConfig,
            Snapshotter snapshotter,
            HighGoConnection connection,
            HighGoEventDispatcher<TableId> dispatcher,
            ErrorHandler errorHandler,
            Clock clock,
            HighGoSchema schema,
            HighGoTaskContext taskContext,
    ...
    public void execute(ChangeEventSourceContext context, HighGoOffsetContext offsetContext)
        ...
        if (!hasStartLsnStoredInContext) {
            offsetContext = HighGoOffsetContext.initialContext(connectorConfig, connection, clock);
        }
                            ...
                            message -> {
                                ...
                                // DML event
                                    ...
                                    if (message.getOperation() != Operation.NOOP) {
                                        tableId = HighGoSchema.parse(message.getTable());
    ...
    public void commitOffset(Map<String, ?> offset) {
        try {
            ...
            final Lsn commitLsn =
                    Lsn.valueOf((Long) offset.get(HighGoOffsetContext.LAST_COMMIT_LSN_KEY));
            final Lsn changeLsn =
                    Lsn.valueOf(
                            (Long)
                                    offset.get(
                                            HighGoOffsetContext.LAST_COMPLETELY_PROCESSED_LSN_KEY));
     ...
     */
    private boolean isInPreSnapshotCatchUpStreaming(HighGoOffsetContext offsetContext) {
