 ...
 */
package io.debezium.connector.highgo.connection.ogproto;
...
import io.debezium.connector.highgo.HighGoStreamingChangeEventSource;
...
public class OgProtoColumnValue extends AbstractColumnValue<PgProto.DatumMessage> {
    ...
    /** A number used by HighGo to define minimum timestamp (inclusive). Defined in timestamp.h */
    private static final long TIMESTAMP_MIN = -211813488000000000L;

    /** A number used by HighGo to define maximum timestamp (exclusive). Defined in timestamp.h */
    ...
    public Object asDecimal() {
        ...
        final String s = asString();
        if (s != null) {
            return HighGoValueConverter.toSpecialValue(s)
    ...
    public OffsetDateTime asOffsetDateTimeAtUtc() {
        if (value.hasDatumInt64()) {
            if (value.getDatumInt64() >= TIMESTAMP_MAX) {
                ...
                return HighGoValueConverter.POSITIVE_INFINITY_OFFSET_DATE_TIME;
            } else if (value.getDatumInt64() < TIMESTAMP_MIN) {
                LOGGER.trace("Infinite(-) value '{}' arrived from database", value.getDatumInt64());
                return HighGoValueConverter.NEGATIVE_INFINITY_OFFSET_DATE_TIME;
            }
    ...
    public Instant asInstant() {
        if (value.hasDatumInt64()) {
            if (value.getDatumInt64() >= TIMESTAMP_MAX) {
                LOGGER.trace("Infinite(+) value '{}' arrived from database", value.getDatumInt64());
                return HighGoValueConverter.POSITIVE_INFINITY_INSTANT;
            } else if (value.getDatumInt64() < TIMESTAMP_MIN) {
                LOGGER.trace("Infinite(-) value '{}' arrived from database", value.getDatumInt64());
                return HighGoValueConverter.NEGATIVE_INFINITY_INSTANT;
            }
    ...
    @Override
    public boolean isArray(HighGoType type) {
    ...
    public Object asArray(
            ...
            HighGoType type,
            String fullType,
            HighGoStreamingChangeEventSource.PgConnectionSupplier connection) {
    ...
    public Object asDefault(
            ...
            HighGoStreamingChangeEventSource.PgConnectionSupplier connection) {
        final HighGoType type = typeRegistry.get(columnType);
