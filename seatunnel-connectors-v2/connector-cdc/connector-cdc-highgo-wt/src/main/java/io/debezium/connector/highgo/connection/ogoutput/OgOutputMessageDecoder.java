 ...
 */
package io.debezium.connector.highgo.connection.ogoutput;
...
public class OgOutputMessageDecoder extends AbstractMessageDecoder {
    ...
    private HighGoConnection connection;
        ...
        switch (messageType) {
            ...
            case TRUNCATE:
                if (decoderContext.getConfig().truncateHandlingMode()
                        == HighGoConnectorConfig.TruncateHandlingMode.INCLUDE) {
    ...
    private void handleRelationMessage(ByteBuffer buffer, TypeRegistry typeRegistry)
        ...
        for (short i = 0; i < columnCount; ++i) {
            ...
            final HighGoType postgresType = typeRegistry.get(columnType);
            boolean key =
    ...
    private List<io.debezium.relational.Column> getTableColumnsFromDatabase(
            HighGoConnection connection, DatabaseMetaData databaseMetadata, TableId tableId)
            throws SQLException {
    ...
    private static List<ReplicationMessage.Column> resolveColumnsFromStreamTupleData(
        ...
        for (short i = 0; i < numberOfColumns; ++i) {
            ...
            final HighGoType columnType = typeRegistry.get(typeName);
                ...
                columns.add(
                        new AbstractReplicationMessageColumn(
                            ...
                            public Object getValue(
                                    HighGoStreamingChangeEventSource.PgConnectionSupplier
                                            connection,
