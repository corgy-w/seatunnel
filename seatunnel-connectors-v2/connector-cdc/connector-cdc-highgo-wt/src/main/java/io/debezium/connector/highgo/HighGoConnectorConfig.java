...
import io.debezium.connector.highgo.connection.ogoutput.OgOutputMessageDecoder;
import io.debezium.connector.highgo.connection.ogproto.OgProtoMessageDecoder;
...
/**
 * The configuration properties for the {@link HighGoConnector}
 *
 * @author Horia Chiorean
 */
public class HighGoConnectorConfig extends RelationalDatabaseConnectorConfig {

    private static final Logger LOGGER = LoggerFactory.getLogger(HighGoConnectorConfig.class);
    private static final String DEFAULT_UNAVAILABLE_VALUE_PLACEHOLDER =
    ...
    public static final Field SLOT_NAME =
            Field.create("slot.name")
                    ...
                    .withValidation(HighGoConnectorConfig::validateReplicationSlotName)
                    .withDescription(
    ...
    public static final Field SSL_SOCKET_FACTORY =
            Field.create(DATABASE_CONFIG_PREFIX + "sslfactory")
                    ...
                    .withImportance(Importance.MEDIUM)
                    .withDescription(
                            "A name of class to that creates SSL Sockets. Use org.highgo.ssl.NonValidatingFactory to disable SSL validation in development environments");
    ...
    public static final Field LOGICAL_DECODING_MESSAGE_PREFIX_EXCLUDE_LIST =
            Field.create("message.prefix.exclude.list")
                    ...
                    .withValidation(
                            Field::isListOfRegex,
                            HighGoConnectorConfig::validateLogicalDecodingMessageExcludeList)
                    .withDescription(
    ...
    public static final Field TRUNCATE_HANDLING_MODE =
            Field.create("truncate.handling.mode")
                    ...
                    .withWidth(Width.MEDIUM)
                    .withImportance(Importance.MEDIUM)
                    .withValidation(HighGoConnectorConfig::validateTruncateHandlingMode)
                    .withDescription(
    ...
    public static final Field TOASTED_VALUE_PLACEHOLDER =
            Field.create("toasted.value.placeholder")
                    ...
                    .withDefault(DEFAULT_UNAVAILABLE_VALUE_PLACEHOLDER)
                    .withImportance(Importance.MEDIUM)
                    .withValidation(HighGoConnectorConfig::validateToastedValuePlaceholder)
                    .withDescription(
                            "Specify the constant that will be provided by Debezium to indicate that "
    ...
    public static final Field XLOG_LOCATION =
            Field.create("xlog.location")
                    .withDisplayName("xlog location")
                    .withType(Type.STRING)
                    .withImportance(Importance.MEDIUM)
                    .withDescription("xlog location");

    public static final Field WAL_SENDER_TIMEOUT =
            Field.create("wal.sender.timeout")
    ...
    public static final Field EXPORT_CSV_PATH =
            Field.create("export.csv.path")
    ...
    public static final Field EXPORT_CSV_PATH_SIZE =
            Field.create("export.csv.path.size")
    ...
    public static final Field EXPORT_FILE_SIZE =
            Field.create("export.file.size")
    ...
    public HighGoConnectorConfig(Configuration config) {
        super(
                config,
        ...
        this.truncateHandlingMode =
                TruncateHandlingMode.parse(
                        config.getString(HighGoConnectorConfig.TRUNCATE_HANDLING_MODE));
        ...
        String hstoreHandlingModeStr = config.getString(HighGoConnectorConfig.HSTORE_HANDLING_MODE);
        ...
        this.intervalHandlingMode =
                IntervalHandlingMode.parse(
                        config.getString(HighGoConnectorConfig.INTERVAL_HANDLING_MODE));
    ...
    protected Duration statusUpdateInterval() {
        return Duration.ofMillis(
                getConfig().getLong(HighGoConnectorConfig.STATUS_UPDATE_INTERVAL_MS));
    }
    ...
    protected Duration xminFetchInterval() {
        return Duration.ofMillis(getConfig().getLong(HighGoConnectorConfig.XMIN_FETCH_INTERVAL));
    }
    ...
    protected SourceInfoStructMaker<? extends AbstractSourceInfo> getSourceInfoStructMaker(
        ...
        switch (version) {
            case V1:
                return new LegacyV1HighGoSourceInfoStructMaker(
                        Module.name(), Module.version(), this);
            default:
                return new HighGoSourceInfoStructMaker(Module.name(), Module.version(), this);
        }
    }
    ...
    public Connection getConnection(HighGoConnectorConfig config) {
        String sourceURL =
                "jdbc:highgo://" + hostname() + ":" + port() + "/" + config.databaseName();
        ...
        try {
            ...
            if (version.startsWith("3.0.")) {
                statement.execute(
                        "alter system set wal_sender_timeout = "
                                + (walSenderTimeout() == null ? 12000 : walSenderTimeout()));
            }*/
        } catch (Exception exp) {
            LOGGER.error("Create HighGo connection failed.", exp);
            throw new SeaTunnelException("Create HighGo connection failed.", exp);
        }
        return connection;
    }
