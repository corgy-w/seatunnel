 ...
 */
package io.debezium.connector.highgo.utils;
...
public class AbstractIncrementalSnapshotContext<T> extends IncrementalSnapshotContext<T> {
    ...
    public static final String DATA_COLLECTIONS_TO_SNAPSHOT_KEY =
            INCREMENTAL_SNAPSHOT_KEY + "_collections";
    ...
    /**
     * The PK of the last record that was passed to Kafka Connect. In case of connector restart the
     * start of the first chunk will be populated from it.
     */
    ...
    public boolean openWindow(String id) {
        if (notExpectedChunk(id)) {
            LOGGER.info(
                    "Received request to open window with id = '{}', expected = '{}', request ignored",
                    id,
                    currentChunkId);
            return false;
        }
    ...
    public boolean closeWindow(String id) {
        if (notExpectedChunk(id)) {
            LOGGER.info(
                    "Received request to close window with id = '{}', expected = '{}', request ignored",
                    id,
                    currentChunkId);
            return false;
        }
     ...
     * as depending on committed offset position some signals can be replayed. In extreme case a
     * signal can be received even when the incremental snapshot was completed just before the
     * restart. Such windowing signals are ignored.
     */
    ...
    private String arrayToSerializedString(Object[] array) {
        try (final ByteArrayOutputStream bos = new ByteArrayOutputStream();
            ...
            throw new DebeziumException(
                    String.format("Cannot serialize chunk information %s", array));
        }
    }
    ...
    private Object[] serializedStringToArray(String field, String serialized) {
        try (final ByteArrayInputStream bis =
            ...
            throw new DebeziumException(
                    String.format("Failed to deserialize '%s' with value '%s'", field, serialized),
                    e);
        }
    }
    ...
    public List<T> addDataCollectionNamesToSnapshot(List<String> dataCollectionIds) {
        final List<T> newDataCollectionIds =
                dataCollectionIds.stream()
    ...
    protected static <U> IncrementalSnapshotContext<U> init(
            AbstractIncrementalSnapshotContext<U> context, Map<String, ?> offsets) {
        final String lastEventSentKeyStr = (String) offsets.get(EVENT_PRIMARY_KEY);
        context.chunkEndPosition =
                (lastEventSentKeyStr != null)
        ...
        context.maximumKey =
                (maximumKeyStr != null)
                        ? context.serializedStringToArray(TABLE_MAXIMUM_KEY, maximumKeyStr)
                        : null;
        ...
        if (dataCollectionsStr != null) {
            context.addDataCollectionNamesToSnapshot(
                    context.stringToDataCollections(dataCollectionsStr));
        }
        return context;
    }
    ...
    public String toString() {
        return "IncrementalSnapshotContext [windowOpened="
                + windowOpened
                + ", chunkEndPosition="
                + Arrays.toString(chunkEndPosition)
                + ", dataCollectionsToSnapshot="
                + dataCollectionsToSnapshot
                + ", lastEventKeySent="
                + Arrays.toString(lastEventKeySent)
                + ", maximumKey="
