...
import static org.apache.seatunnel.connectors.seatunnel.hudi.config.HudiOptions.DATA_SAVE_MODE;
...
import static org.apache.seatunnel.connectors.seatunnel.hudi.config.HudiOptions.SCHEMA_SAVE_MODE;
...
public class HudiSinkFactory implements TableSinkFactory {
    ...
    public TableSink createSink(TableSinkFactoryContext context) {
        ...
        return () -> new HudiSink(config, catalogTable);
    }

    private CatalogTable renameCatalogTable(ReadonlyConfig config, CatalogTable catalogTable) {
        TableIdentifier tableId = catalogTable.getTableId();
        String tableName = config.getOptional(TABLE_NAME).orElse(tableId.getTableName());
        String database = config.getOptional(DATABASE).orElse(tableId.getDatabaseName());
        TableIdentifier newTableId;
        if (StringUtils.isBlank(database)) {
            newTableId = TableIdentifier.of(tableId.getCatalogName(), null, tableName);
        } else {
            newTableId = TableIdentifier.of(tableId.getCatalogName(), database, tableName);
        }
        Map<String, String> options = new HashMap<>(catalogTable.getOptions());
        config.getOptional(TABLE_TYPE)
                .ifPresent(value -> options.put(TABLE_TYPE.key(), value.name()));
        config.getOptional(RECORD_KEY_FIELDS)
                .ifPresent(value -> options.put(RECORD_KEY_FIELDS.key(), value));
        List<String> partitionKeys = catalogTable.getPartitionKeys();
        if (config.getOptional(PARTITION_FIELDS).isPresent()) {
            String partitionFields = config.get(PARTITION_FIELDS);
            partitionKeys =
                    StringUtils.isBlank(partitionFields)
                            ? partitionKeys
                            : Arrays.asList(partitionFields.split("\\s*,\\s*"));
        }
