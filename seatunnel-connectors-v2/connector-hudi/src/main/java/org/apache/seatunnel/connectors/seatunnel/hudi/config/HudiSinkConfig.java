...
public class HudiSinkConfig implements Serializable {
    ...
    private String kerberosPrincipalFile;
    ...
    public static HudiSinkConfig of(ReadonlyConfig config) {
        ...
        builder.useKerberos(config.get(HudiOptions.USE_KERBEROS));
        builder.krb5Path(config.getOptional(HudiOptions.KRB5_PATH).orElse(null));
        builder.kerberosPrincipal(config.getOptional(HudiOptions.KERBEROS_PRINCIPAL).orElse(null));
        builder.kerberosPrincipalFile(
                config.getOptional(HudiOptions.KERBEROS_PRINCIPAL_FILE).orElse(null));
        builder.schemaSaveMode(config.get(HudiOptions.SCHEMA_SAVE_MODE));
        builder.dataSaveMode(config.get(HudiOptions.DATA_SAVE_MODE));
