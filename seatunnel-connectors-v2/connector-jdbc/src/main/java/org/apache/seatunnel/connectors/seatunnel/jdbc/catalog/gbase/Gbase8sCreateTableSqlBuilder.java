...
public class Gbase8sCreateTableSqlBuilder {
    private List<Column> columns;
    private PrimaryKey primaryKey;
    private String sourceCatalogName;

    public Gbase8sCreateTableSqlBuilder(CatalogTable catalogTable) {
    ...
    String buildColumnSql(Column column) {
        ...
        } else if (sourceCatalogName.equalsIgnoreCase(DatabaseIdentifier.GBASE_8S)
                && StringUtils.isNotBlank(column.getSourceType())) {
            columnType = column.getSourceType();
        } else {
            columnType = Gbase8sTypeConverter.INSTANCE.reconvert(column).getColumnType();
        }
