...
public class MaxcomputeSourceSplitEnumerator
    ...
    private void discoverySplits() throws TunnelException {
        ...
        for (SourceTableInfo sourceTableInfo : sourceTableInfos.values()) {
            ...
            Set<MaxcomputeSourceSplit> finalAllSplit = splits;
            splits.forEach(split -> split.setSplitCount(finalAllSplit.size()));
            // reindex hashcode
            splits = new HashSet<>(splits);
            assignedSplits.forEach(splits::remove);
            eventRecorder.addTableSplit(
                    sourceTableInfo.getCatalogTable().getTablePath(), splits.size());
